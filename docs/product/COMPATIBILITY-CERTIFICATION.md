...
| PHP | Runtime | **Certified/recommended: PHP 8.5.x.** Minimum supported: PHP 8.3. Supported/certified range: 8.3–8.5.x. PHP 8.1 and 8.2 are not supported. | Yes | CI: **8.3 Minimum Supported** (blocking); **8.4 Compatibility**; **8.5 CETECH Production Target** (blocking PHPUnit, MariaDB, WP/Woo/HPOS/Store API) | Plugin parse/PHPUnit historically run on local PHP 8.5; RC.11/RC.12 GitHub jobs were 8.1 lint + 8.2 PHPUnit and must not be misread as current support. Full WoodMart/B2BKing/FOX stack on 8.5 remains Pilot evidence. | Mandatory: recommended WP/Woo floor, not oldest bootable PHP |
