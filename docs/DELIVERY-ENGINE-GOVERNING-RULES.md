...
**Current QA-candidate identity:** `1.0.0-rc.5-qa.3` (master; schema `4`; not a final RC.5 tag)  
...
9. Tagged **`1.0.0-rc.4`** remains the protected published baseline (schema **3** on FLAIROC until an authorised install). Master may carry an approved QA-candidate identity such as **`1.0.0-rc.5-qa.3`** during Stage 14H. Schema target is **`4`**. Stage 14 flags default **OFF**. Do not create the final `1.0.0-rc.5` tag, overwrite RC.4, or claim owner QA passed without the owner’s physical confirmation.
