...
| Next stage | **STOP** before packaging. Bulk background-execution portability is implemented and locally verified on `1.0.0-dev.bulk.8` (schema `5`, no schema 6). Artifact `docs/POST-RC6-BULK-BACKGROUND-PORTABILITY.md`. Owner physical QA of the previous packaged `1.0.0-dev.bulk.7` ZIP may continue, but that ZIP does **not** include this repair. Do **not** begin Stage 15. Do **not** modify FLAIROC. Do **not** retag `v1.0.0-rc.5` or `v1.0.0-rc.6`. Cursor must **not** install a package. Do **not** keep swapping R1 ZIP ↔ Bulk ZIP. |
...
| `docs/POST-RC6-CONSOLIDATED-DEVELOPMENT-STREAM.md` | Combined post-RC.6 Bulk Tools + R1 stream record (packaged identity `1.0.0-dev.bulk.7`; current tree `1.0.0-dev.bulk.8`) |
