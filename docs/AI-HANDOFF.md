...
| Post-RC.7 fulfilment correctness | **PACKAGED / NOT DEPLOYED** — `1.0.0-dev.fulfilment.3` on `feat/post-rc7-fulfilment-correctness`. Cart presentation repair after fulfilment.2 architecture acceptance. ZIP record: `docs/POST-RC7-FULFILMENT-CORRECTNESS-QA.md`. Repair note: `docs/POST-RC7-FULFILMENT-CORRECTNESS-CART-PRESENTATION.md`. Schema **`5`**. RC.7 tag/FLAIROC untouched. Owner physical QA: **Scenario 1 cart recheck only** (Pickup-only + mixed Delivery + Pickup). |
