...
**Post-RC.6 Bulk Tools:** authorised separately from Stage 14/15. Schema target in **this tree** is **`5`**. Architecture: `docs/POST-RC6-BULK-TOOLS-ARCHITECTURE.md`. Implementation: `docs/POST-RC6-BULK-TOOLS-IMPLEMENTATION.md`. Automated qualification PASS plus pre-QA cleanup recorded in `docs/POST-RC6-BULK-TOOLS-QUALIFICATION.md`. Untagged owner-QA package identity: `1.0.0-dev.bulk.2`. Cursor must not install it on training.cetechbpa.com, FLAIROC, or another owner site.  
