...
| Post-RC.7 fulfilment correctness | **PACKAGED / NOT DEPLOYED** — `1.0.0-dev.fulfilment.2` on `feat/post-rc7-fulfilment-correctness`. Scenario 1 admin repair. Artifact `docs/POST-RC7-FULFILMENT-CORRECTNESS-ADMIN-REPAIR.md`. ZIP record `docs/POST-RC7-FULFILMENT-CORRECTNESS-QA.md`. Schema **`5`**. RC.7 tag/FLAIROC untouched. Owner physical QA: **Scenario 1 only** until it passes. |
