...
**Plugin version:** `1.0.0-rc.5-qa.6` on master (QA retest candidate). Tagged/protected published baseline remains `1.0.0-rc.4` (schema `3` tag). QA.5 failed owner QA on FLAIROC: amount-only refund did not create Needs Attention refund review. Do not retag RC.3 or RC.4. Do not create `v1.0.0-rc.5` yet.  
**Schema target:** `4` (`cetech_de_db_version`) — in this tree and in the QA.6 ZIP. QA.1 already migrated FLAIROC to schema 4; do not create schema 5.  
...
| Stage 14H-QA5-R1 QA.6 refund review repair | **RC.5-QA.6 PREPARED FOR OWNER INSTALL** — amount-only / unproven refunds keep shipment status and appear in Needs Attention as refund review. Explicit full item-quantity refunds before dispatch still auto-cancel. Package `cetech-woocommerce-delivery-engine-1.0.0-rc.5-qa.6.zip`. **Not** owner QA pass. **Not** final RC.5. FLAIROC **not modified**. No schema 5. |
...
| Runtime on FLAIROC | **Required production features ON**; deferred features **OFF**; **COD OFF** on production storefront policy. Schema **4** (QA.1 migration already ran). QA.5 is the current live plugin until the owner installs QA.6. Master/QA.6 schema target remains **4**. |
| Shipments / tracking / timeline | **Stage 14 V1 implemented and Settings-activatable** (`enable_shipment_records` / `enable_tracking_links`, default OFF). Timeline remains reserved. **RC.5-QA.1 FAILED. RC.5-QA.2 FAILED (inheritance). RC.5-QA.3 PREPARED (not physically retested alone). RC.5-QA.4 OWNER PHYSICAL PASS. RC.5-QA.5 FAILED OWNER QA (amount-only refund review). RC.5-QA.6 PREPARED FOR OWNER INSTALL.** No carrier automation, no shipment emails, no customer timeline. |
...
Stage 14C–14F implemented shipment planning, staff workspace, tracking, customer cards, and operations behind flags. Stage 14G / G-R1 qualified the work, including real MariaDB schema 3→4. Stage 14H prepared **`1.0.0-rc.5-qa.1`**, which **FAILED** owner QA (false checkout failure from a customer-summary fatal; COD processing created shipments without payment confirmation). Stage 14H-QA1-R1 prepared **`1.0.0-rc.5-qa.2`**, which **FAILED** owner QA because untouched products did not inherit a usable Site-wide configuration (optional ECR fields wrongly fail-closed the whole EffectiveConfiguration). Stage 14H-QA2-R1 prepared **`1.0.0-rc.5-qa.3`** (inheritance repair; not physically retested). Stage 14H-QA3-R1 prepared **`1.0.0-rc.5-qa.4`**, which the owner physically tested on FLAIROC: inheritance, badges, Track button, staff actor resolution, COD no premature shipment, and payment_complete exactly one shipment all **PASSED**. Stage 14H-QA4-R1 prepared **`1.0.0-rc.5-qa.5`**, which the owner physically tested on FLAIROC: COD action-required and manual historical shipment creation worked, but an amount-only refund on order **39749** did not create Needs Attention refund review. Stage 14H-QA5-R1 prepared **`1.0.0-rc.5-qa.6`**: unproven/amount-only refunds keep shipment status and appear as `refund_requires_review`; explicit full item-quantity refunds before dispatch still auto-cancel. **RC.5-QA.6 PREPARED FOR OWNER INSTALL.** Do not claim owner QA passed. Do not finalize RC.5. FLAIROC was not modified by this QA.6 preparation. Training remains draft.
