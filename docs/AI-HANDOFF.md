...
| Post-RC.7 fulfilment correctness | **PACKAGED / NOT DEPLOYED** — `1.0.0-dev.fulfilment.4` on `feat/post-rc7-fulfilment-correctness`. Mixed-cart Pickup package no longer renders `Shipping to` / `Change address`. ZIP record: `docs/POST-RC7-FULFILMENT-CORRECTNESS-QA.md`. Repair note: `docs/POST-RC7-FULFILMENT-CORRECTNESS-CART-PRESENTATION.md`. Schema **`5`**. RC.7 tag/FLAIROC untouched. Owner physical QA: **one mixed Delivery + Pickup cart screenshot only**. |
