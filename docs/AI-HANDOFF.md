...
**Post-RC.12 issue #39:** OPEN — Cart/checkout delivery-address actions and per-item editor need clearer hierarchy and compact UX. Current candidate identity **`1.0.0-dev.address-ux.1`** on `fix/cart-checkout-address-ux`, based on protected `master` `ed3753d73e262d8c7467fa936e3e36060960e58c` (Issue #38 CLOSED; PR #40 MERGED). Schema `6`. **Not RC.13. Not deployed. Do not merge until owner/ChatGPT technical review.** Artifact `docs/POST-RC12-CART-CHECKOUT-ADDRESS-UX.md` after runtime CI SUCCESS. Training remains on `1.0.0-dev.pdp-precision.2`. RC.12 tag/ZIP remain immutable.  
...
| `docs/POST-RC12-PDP-LOCATION-PRECISION.md` | Issue #38 PDP location precision / exact delivery quote candidate (`1.0.0-dev.pdp-precision.2`; frozen `.1` ZIP; not RC.13; not deployed) |
