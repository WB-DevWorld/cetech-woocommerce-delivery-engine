...
**Post-RC.9 per-item customer context:** **IN THIS TREE as `1.0.0-dev.peritem.1`** — Stages 1–2 domain + cart-state foundation only. No storefront UI. Schema remains **`5`**. Not RC.10. Not Stage 15. FLAIROC / training / WCFM / WPML untouched. RC.9 tag untouched.  
...
**Git:** `feat/post-rc9-cart-state` (post-RC.9 cart-state reconciliation development). Historical `feat/post-rc8-integrations` promoted to `v1.0.0-rc.9`. Historical `feat/post-rc7-fulfilment-correctness` promoted to `v1.0.0-rc.8`. Historical `integration/post-rc6-bulk-r1`, `feat/post-rc6-bulk-tools`, and `fix/post-rc6-admin-setup-defects` not rewritten. Unrelated RC.6 security-audit leftovers preserved on `wip/rc6-adversarial-security-audit` @ `a88f28048afcc84456e6933351c71b01a9838c5a`. RC.6 tag `v1.0.0-rc.6`, RC.7 tag `v1.0.0-rc.7`, RC.8 tag `v1.0.0-rc.8`, and RC.9 tag `v1.0.0-rc.9` **untouched**.  
...
| Post-RC.9 cart-state reconciliation | **INCLUDED** — live cart-state refresh; `docs/POST-RC9-CART-STATE-RECONCILIATION.md`. Schema **`5`**. |
| Post-RC.9 Blocks line snapshot | **INCLUDED** — native Blocks/Store API line snapshots. Artifact `docs/POST-RC9-BLOCKS-LINE-SNAPSHOT.md`. |
| Next stage | **STOP.** Tagged `1.0.0-rc.9` remains the protected published baseline. Training in-place install was **not** completed from Cursor (no training SSH/WP-CLI/browser admin path). This qualification assembly now includes post-RC.9 cart-state reconciliation and Blocks line snapshots as implemented-but-unreleased candidate history. It is not RC.10. Do **not** begin Stage 15. Do **not** modify FLAIROC. Do **not** retag `v1.0.0-rc.9` or earlier. After a human in-place training install, confirm Active / `1.0.0-rc.9` / schema `5` / cart `12` / checkout `13` / layouts 1482·1496·1500 publish. Cursor must **not** install qualification packages. |
