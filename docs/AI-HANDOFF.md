...
**Plugin version:** `1.0.0-rc.5-qa.4` on master (QA retest candidate). Tagged/protected published baseline remains `1.0.0-rc.4` (schema `3` tag). FLAIROC currently still has a failed QA candidate installed (QA.1 and/or QA.2) with Stage 14 flags OFF. QA.3 was prepared but not physically installed. Do not retag RC.3 or RC.4. Do not create `v1.0.0-rc.5` yet.  
**Schema target:** `4` (`cetech_de_db_version`) — in this tree and in the QA.4 ZIP. QA.1 already migrated FLAIROC to schema 4; do not create schema 5.  
...
| Stage 14H-QA3-R1 QA.4 polish | **RC.5-QA.4 PREPARED FOR RETEST** — QA.3 inheritance repair plus staff History identity, customer Track shipment button, and Needs Attention / per-user Shipments activity badges. Package `cetech-woocommerce-delivery-engine-1.0.0-rc.5-qa.4.zip`. **Not** owner QA pass. **Not** final RC.5. FLAIROC **not modified**. No schema 5. |
...
| Runtime on FLAIROC | **Required production features ON**; deferred features **OFF**; **COD OFF** on production storefront policy. Schema **4** (QA.1 migration already ran). Plugin currently a **failed QA candidate** until the owner installs QA.4. Master/QA.4 schema target remains **4**. |
| Shipments / tracking / timeline | **Stage 14 V1 implemented and Settings-activatable** (`enable_shipment_records` / `enable_tracking_links`, default OFF). Timeline remains reserved. **RC.5-QA.1 FAILED OWNER QA. RC.5-QA.2 FAILED OWNER QA (inheritance). RC.5-QA.3 PREPARED (not physically retested). RC.5-QA.4 PREPARED FOR RETEST.** Flags currently OFF on FLAIROC. No carrier automation, no shipment emails, no customer timeline. |
...
| Next stage | **WAIT FOR OWNER PHYSICAL RETEST** of `1.0.0-rc.5-qa.4`. Then Stage 14H-FINAL only if owner QA passes. Do not create `v1.0.0-rc.5` yet. Do not claim owner QA passed. |
...
| Package | QA retest candidate `cetech-woocommerce-delivery-engine-1.0.0-rc.5-qa.4.zip` (SHA-256 in `docs/STAGE-14H-RC5-OWNER-QA.md`). Failed QA.1/QA.2 and evidence QA.3 ZIPs remain immutable. RC.4 rollback ZIP remains the protected published package. |
...
| `docs/STAGE-14H-RC5-OWNER-QA.md` | Stage 14H RC.5 owner QA — **QA.1 FAIL**; **QA.2 FAIL** (inheritance); **QA.3 prepared (not physically retested)**; **QA.4 PREPARED FOR RETEST** |
