...
**Plugin version:** development identity **`1.0.0-dev.blocks.1`** on `feat/post-rc8-integrations` (starts from tagged `v1.0.0-rc.8`). Tagged/protected published baseline remains **`1.0.0-rc.8`** (schema `5`, tag `v1.0.0-rc.8`). Packaged for owner QA — **physical QA pending**. Do not retag RC.2–RC.8. Do not rebuild the RC.6, RC.7, or RC.8 ZIP. Do not begin Stage 15. FLAIROC must not be modified. Historical tagged **`1.0.0-rc.7`** (schema `5`) and **`1.0.0-rc.6`** (schema `4`) remain untouched.  
**Post-RC.8 Settings honesty + Cart/Checkout Blocks:** **PACKAGED / PHYSICAL QA PENDING** — identity `1.0.0-dev.blocks.1`, schema **`5`**. Canonical record: `docs/POST-RC8-BLOCKS-IMPLEMENTATION.md`. QA package: `docs/POST-RC8-BLOCKS-QA.md`. Audit: `docs/POST-RC8-INTEGRATIONS-COMPATIBILITY-AUDIT.md`. Classic checkout unchanged. WPML/WCML/WCFM/VitePOS adapters not implemented. FLAIROC not modified. RC.8 tag untouched.  
...
| Post-RC.8 Settings honesty + Blocks | **PACKAGED / PHYSICAL QA PENDING** — `1.0.0-dev.blocks.1`, schema `5`. Settings honesty + Store API Cart/Checkout Blocks adapter. Artifact `docs/POST-RC8-BLOCKS-IMPLEMENTATION.md`. QA ZIP `docs/POST-RC8-BLOCKS-QA.md`. Not Stage 15. FLAIROC not modified. |
| Next stage | **STOP.** QA identity `1.0.0-dev.blocks.1` is packaged. Owner physical QA is three Blocks checks on training.cetechbpa.com, then restore Classic pages. Cursor must **not** install on FLAIROC. Do **not** begin Stage 15. Do **not** retag `v1.0.0-rc.5`–`v1.0.0-rc.8`. Do **not** create RC.9 until physical QA is accepted. |
...
| `docs/POST-RC8-BLOCKS-QA.md` | `1.0.0-dev.blocks.1` owner-QA package record — **PHYSICAL QA PENDING** |
