...
**Plugin version:** `1.0.0-rc.5-qa.5` on master (QA retest candidate). Tagged/protected published baseline remains `1.0.0-rc.4` (schema `3` tag). QA.4 was physically installed/tested on FLAIROC and **PASSED** the listed inheritance, badge, Track button, staff-actor, and COD payment-gate checks. Do not retag RC.3 or RC.4. Do not create `v1.0.0-rc.5` yet.  
**Schema target:** `4` (`cetech_de_db_version`) — in this tree and in the QA.5 ZIP. QA.1 already migrated FLAIROC to schema 4; do not create schema 5.  
...
| Stage 14H-QA4-R1 QA.5 operational completeness | **RC.5-QA.5 PREPARED FOR OWNER INSTALL** — COD action-required Needs Attention, staff manual shipment creation from the historical order snapshot, unique staff History identity, contained customer shipment cards. Package `cetech-woocommerce-delivery-engine-1.0.0-rc.5-qa.5.zip`. **Not** owner QA pass. **Not** final RC.5. FLAIROC **not modified**. No schema 5. |
...
| Runtime on FLAIROC | **Required production features ON**; deferred features **OFF**; **COD OFF** on production storefront policy. Schema **4** (QA.1 migration already ran). QA.4 was physically tested and passed. Plugin currently still the QA.4 runtime until the owner installs QA.5. Master/QA.5 schema target remains **4**. |
| Shipments / tracking / timeline | **Stage 14 V1 implemented and Settings-activatable** (`enable_shipment_records` / `enable_tracking_links`, default OFF). Timeline remains reserved. **RC.5-QA.1 FAILED. RC.5-QA.2 FAILED (inheritance). RC.5-QA.3 PREPARED (not physically retested alone). RC.5-QA.4 OWNER PHYSICAL PASS. RC.5-QA.5 PREPARED FOR OWNER INSTALL.** No carrier automation, no shipment emails, no customer timeline. |
...
| Next stage | **WAIT FOR OWNER PHYSICAL INSTALL/TEST** of `1.0.0-rc.5-qa.5`. Then Stage 14H-FINAL only if owner QA passes. Do not create `v1.0.0-rc.5` yet. Do not claim owner QA passed. |
...
| Package | QA retest candidate `cetech-woocommerce-delivery-engine-1.0.0-rc.5-qa.5.zip` (`1037829` bytes, SHA-256 `8a142194bc548eab29213b953d37c7441684f78d2ebc1315d85e25d637735212`, source `9d92be6`). QA.4 ZIP remains immutable (physical PASS). Failed QA.1/QA.2 and evidence QA.3 ZIPs remain immutable. RC.4 rollback ZIP remains the protected published package. |
...
Stage 14C–14F implemented shipment planning, staff workspace, tracking, customer cards, and operations behind flags. Stage 14G / G-R1 qualified the work, including real MariaDB schema 3→4. Stage 14H prepared **`1.0.0-rc.5-qa.1`**, which **FAILED** owner QA (false checkout failure from a customer-summary fatal; COD processing created shipments without payment confirmation). Stage 14H-QA1-R1 prepared **`1.0.0-rc.5-qa.2`**, which **FAILED** owner QA because untouched products did not inherit a usable Site-wide configuration (optional ECR fields wrongly fail-closed the whole EffectiveConfiguration). Stage 14H-QA2-R1 prepared **`1.0.0-rc.5-qa.3`** (inheritance repair; not physically retested). Stage 14H-QA3-R1 prepared **`1.0.0-rc.5-qa.4`**, which the owner physically tested on FLAIROC: inheritance, badges, Track button, staff actor resolution, COD no premature shipment, and payment_complete exactly one shipment all **PASSED**. Stage 14H-QA4-R1 prepared **`1.0.0-rc.5-qa.5`**: legitimate COD orders appear as Needs Attention action-required tasks; staff create the shipment from the historical order snapshot; later payment confirmation remains idempotent; staff History uses unique name + User ID; customer View Order uses one contained card per shipment. Administrators can enable shipment records and customer tracking links in Settings; defaults remain **OFF**; timeline stays reserved. **RC.5-QA.5 PREPARED FOR OWNER INSTALL.** Do not claim owner QA passed. Do not finalize RC.5. FLAIROC was not modified by this QA.5 preparation. Training remains draft.
...
| `docs/STAGE-14H-RC5-OWNER-QA.md` | Stage 14H RC.5 owner QA — **QA.1 FAIL**; **QA.2 FAIL** (inheritance); **QA.3 prepared (not physically retested)**; **QA.4 OWNER PHYSICAL PASS**; **QA.5 PREPARED FOR OWNER INSTALL** |
