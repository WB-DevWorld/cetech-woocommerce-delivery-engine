...
**Post-RC.12 issue #32:** OPEN — Overview Needs Attention tile counts catalog setup only, while the menu badge already uses the canonical aggregate. Candidate identity **`1.0.0-dev.attention-count.1`** on `fix/needs-attention-count-contract`, based on protected `master` `d66e5e366612468d7e5b4874e1a4290c5d2582cc` (Issue #31 CLOSED; PR #42 MERGED; post-merge CI `35756300042` SUCCESS). Schema `6`. **Not RC.13. Not deployed. Do not merge until owner/ChatGPT technical review.** Artifact `docs/POST-RC12-NEEDS-ATTENTION-COUNT.md`. Training remains `1.0.0-dev.shipment-order-read.1`. Do not deploy attention-count.1. RC.12 tag/ZIP remain immutable.  
