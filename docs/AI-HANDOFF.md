...
| Next stage | **STOP.** RC.9 ZIP is on the Desktop. Training in-place install was **not** completed from Cursor (no training SSH/WP-CLI/browser admin path). Do **not** begin Stage 15. Do **not** modify FLAIROC. Do **not** retag `v1.0.0-rc.8` or earlier. After a human in-place training install, confirm Active / `1.0.0-rc.9` / schema `5` / cart `12` / checkout `13` / layouts 1482·1496·1500 publish. Beta testers vet Classic storefront usage. Do not manufacture another large owner-QA cycle. |
