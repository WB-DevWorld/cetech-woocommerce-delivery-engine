...
**Post-RC.9 per-item customer context:** **IN THIS TREE as `1.0.0-dev.peritem.1`** — Stages 1–6: domain + cart-state foundation, package-destination quoting, Classic PDP/cart/checkout UX. Blocks location editor **not** implemented. Schema remains **`5`**. Not RC.10. Not Stage 15. FLAIROC / training / WCFM / WPML untouched. RC.9 tag untouched. Canonical record: `docs/POST-RC9-PER-ITEM-CONTEXT.md`.  
...
| Post-RC.9 per-item customer context | **IN DEVELOPMENT `1.0.0-dev.peritem.1`** — Stages 1–6 (quoting + Classic PDP/cart/checkout). Blocks location UI pending. Schema **`5`**. Not RC.10. Artifact: `docs/POST-RC9-PER-ITEM-CONTEXT.md`. |
| Next stage | **STOP.** Tagged `1.0.0-rc.9` remains the protected published baseline. Training in-place install was **not** completed from Cursor (no training SSH/WP-CLI/browser admin path). This qualification assembly now includes post-RC.9 cart-state reconciliation, Blocks line snapshots, per-item customer-context foundation, and Classic per-item destination UX as implemented-but-unreleased candidate history. WPML is not included. It is not RC.10. Do **not** begin Stage 15. Do **not** modify FLAIROC. Do **not** retag `v1.0.0-rc.9` or earlier. After a human in-place training install, confirm Active / `1.0.0-rc.9` / schema `5` / cart `12` / checkout `13` / layouts 1482·1496·1500 publish. Cursor must **not** install qualification packages. |
