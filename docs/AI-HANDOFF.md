...
**Plugin version:** development tree **`1.0.0-dev.peritem.1`** on `feat/post-rc9-per-item-context` (schema `5`). Combines accepted `1.0.0-dev.cartstate.1` (`39f61c1`) and `1.0.0-dev.blocks-snapshot.1` (`54c9894`). Tagged/protected published baseline remains **`1.0.0-rc.9`** (schema `5`, tag `v1.0.0-rc.9`, source `e6bc7fba16d9d7b96682f2945c518a33a9a16cd5`). Do not retag RC.2–RC.9. Do not rebuild the RC.9 ZIP. Do not begin Stage 15. FLAIROC must not be modified. Historical tagged **`1.0.0-rc.8`** / **`1.0.0-rc.7`** (schema `5`) and **`1.0.0-rc.6`** (schema `4`) remain untouched.  
...
**Post-RC.9 per-item customer context:** **IN THIS TREE as `1.0.0-dev.peritem.1`** — Stages 1–6 Classic complete: domain + cart-state foundation, package-destination quoting, Classic same-product two-destination cart (Accra 15 + Kumasi 22 in one cart), consolidation / split / pickup / use-for-all / checkout policy, variable location-first, ETA wording. Blocks location editor **not** implemented. Schema remains **`5`**. Not RC.10. Not Stage 15. FLAIROC / training / WCFM / WPML untouched. RC.9 tag untouched. Canonical record: `docs/POST-RC9-PER-ITEM-CONTEXT.md`.  
...
**Git:** `feat/post-rc9-per-item-context` (post-RC.9 per-item customer context foundation). Cherry-picks: cartstate.1 `39f61c1` → `f6b5c12`; blocks-snapshot.1 `54c9894` → `729f4e8`. Historical `feat/post-rc8-integrations` promoted to `v1.0.0-rc.9`. Historical `feat/post-rc7-fulfilment-correctness` promoted to `v1.0.0-rc.8`. Historical `integration/post-rc6-bulk-r1`, `feat/post-rc6-bulk-tools`, and `fix/post-rc6-admin-setup-defects` not rewritten. Unrelated RC.6 security-audit leftovers preserved on `wip/rc6-adversarial-security-audit` @ `a88f28048afcc84456e6933351c71b01a9838c5a`. RC.6 tag `v1.0.0-rc.6`, RC.7 tag `v1.0.0-rc.7`, RC.8 tag `v1.0.0-rc.8`, and RC.9 tag `v1.0.0-rc.9` **untouched**.  
...
| Post-RC.9 per-item customer context | **`1.0.0-dev.peritem.1` Stages 1–6 Classic complete** — one-cart Accra 15 + Kumasi 22; Classic flows 1–10; variable location-first; ETA wording. Blocks location UI pending. Schema **`5`**. Not RC.10. Artifact: `docs/POST-RC9-PER-ITEM-CONTEXT.md`. |
| Next stage | **STOP.** Tagged `1.0.0-rc.9` remains the protected published baseline. Training in-place install was **not** completed from Cursor (no training SSH/WP-CLI/browser admin path). This qualification assembly now includes post-RC.9 cart-state reconciliation, Blocks line snapshots, per-item destinations, and Classic multi-destination completion as implemented-but-unreleased candidate history. WPML is not included. It is not RC.10. Do **not** begin Stage 15. Do **not** modify FLAIROC. Do **not** retag `v1.0.0-rc.9` or earlier. After a human in-place training install, confirm Active / `1.0.0-rc.9` / schema `5` / cart `12` / checkout `13` / layouts 1482·1496·1500 publish. Cursor must **not** install qualification packages. |
