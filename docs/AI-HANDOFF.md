...
| Post-RC.8 Settings honesty + Blocks | **BLOCKS.3 REPAIR** — `1.0.0-dev.blocks.3`, schema `5`. Ordered matched-area pricing fallback so Accra + Greater Accra no longer require duplicated Air rates. Artifact `docs/POST-RC8-BLOCKS-3-MATCHED-AREA-PRICING.md`. Not Stage 15. FLAIROC not modified. |
| Next stage | **STOP.** Repair identity `1.0.0-dev.blocks.3`. Owner QA is the International Air Blocks path that previously returned `no_matching_rate_card`, plus confirmation that mixed In Store Blocks still has no false warning. Do **not** create RC.9. Do **not** modify FLAIROC. Do **not** retag `v1.0.0-rc.8`. |
