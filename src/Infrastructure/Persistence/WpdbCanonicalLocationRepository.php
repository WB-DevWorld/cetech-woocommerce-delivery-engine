...
final class WpdbCanonicalLocationRepository extends AbstractWpdbRepository implements CanonicalLocationRepositoryInterface {
	...
	public function find_exact_child( string $country_code, ?int $parent_id, string $normalized_name, ?GeographyLocationType $type = null, ?int $include_generation = null ): ?CanonicalLocation {
		...
		if ( null !== $include_generation && $include_generation > 0 ) {
	...
	public function promote_generation( int $generation ): int {
		...
		global $wpdb;
		$table = $this->table_name();
		$sql   = "SELECT * FROM `{$table}` WHERE draft_json IS NOT NULL AND draft_json != ''";
		...
		$activated = $wpdb->query(
			$wpdb->prepare(
				"UPDATE `{$table}` SET status = %s, updated_at = %s WHERE generation = %d AND status = %s",
		...
		return is_int( $activated ) ? $activated : 0;
	}
