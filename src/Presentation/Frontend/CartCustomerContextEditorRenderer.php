...
final class CartCustomerContextEditorRenderer {
	...
	public function render_editor(
		...
		$html  = '<div class="cetech-de-cart-context" id="' . esc_attr( $anchor ) . '" data-cetech-de-ui-anchor="' . esc_attr( $anchor ) . '" data-cetech-de-address-complete="' . ( $address_complete ? '1' : '0' ) . '" data-cetech-de-address-required="' . ( $address_required ? '1' : '0' ) . '" data-cetech-de-fulfilment="' . esc_attr( $choice ) . '">';
	...
	private function render_form(
		...
		$html .= $this->render_destination_section( $uid, $matching, $has_matching, $pickup_hidden );
		$html .= $this->render_method_section( $uid, $options, $selected_key );
		$html .= $this->render_address_section( $uid, $address, $pickup_hidden );
		$html .= $this->render_recipient_section( $uid, $address, $pickup_hidden );

		if ( $qty > 1 && FulfilmentChoice::Delivery->value === sanitize_key( (string) ( $intent['fulfilment_choice'] ?? '' ) ) ) {
			$html .= $this->render_quantity_section( $uid, $qty );
		}

		$html .= '<p class="cetech-de-cart-context__actions">';
		$html .= '<button type="submit" class="button cetech-de-cart-context__save">'
		...
		if ( FulfilmentChoice::Delivery->value === sanitize_key( (string) ( $intent['fulfilment_choice'] ?? '' ) ) ) {
			$html .= ' <button type="submit" class="cetech-de-cart-context__use-for-all cetech-de-cart-context__action--secondary" name="'
	...
	private function render_destination_section( string $uid, ?MatchingLocation $matching, bool $has_matching, string $pickup_hidden ): string {
	 ...
	 * @param list<ProductDeliveryOption> $options
	 */
	private function render_method_section( string $uid, array $options, string $selected_key ): string {
		...
		if ( 1 === count( $options ) ) {
			...
			$html  .= '<input type="hidden" name="' . esc_attr( CartDeliverySelectionCapture::POST_FIELD ) . '" value="'
				. esc_attr( $option->display_key ) . '" data-cetech-de-choice="' . esc_attr( $option->fulfilment_choice ) . '" />';
		...
		$html     .= '<select id="' . esc_attr( $select_id ) . '" name="' . esc_attr( CartDeliverySelectionCapture::POST_FIELD ) . '">';
	 ...
	 * @param mixed $address
	 */
	private function render_address_section( string $uid, $address, string $pickup_hidden ): string {
		...
		$html .= $this->text_field( $uid . '-address-1', 'cetech_de_address_1', CustomerStorefrontCopy::address_line_1(), $line1, true );
		...
		$html .= $this->text_field( $uid . '-address-2', 'cetech_de_address_2', CustomerStorefrontCopy::address_line_2_optional(), $line2 );
	 ...
	 * @param mixed $address
	 */
	private function render_recipient_section( string $uid, $address, string $pickup_hidden ): string {
		...
		$html .= $this->text_field( $uid . '-first', 'cetech_de_first_name', __( 'First name', 'cetech-woocommerce-delivery-engine' ), $first );
		$html .= $this->text_field( $uid . '-last', 'cetech_de_last_name', __( 'Last name', 'cetech-woocommerce-delivery-engine' ), $last );
		$html .= $this->text_field( $uid . '-phone', 'cetech_de_phone', __( 'Phone', 'cetech-woocommerce-delivery-engine' ), $phone );
		$html .= $this->text_field( $uid . '-company', 'cetech_de_company', __( 'Company', 'cetech-woocommerce-delivery-engine' ), $company );
		$html .= '</div></details></div>';

		return $html;
	}

	private function render_quantity_section( string $uid, int $qty ): string {
	...
	private function text_field( string $id, string $name, string $label, string $value, bool $required_address = false ): string {
		...
		return '<p class="cetech-de-cart-context__field">'
			...
			. '<input type="text" id="' . esc_attr( $id ) . '" name="' . esc_attr( $name ) . '" value="' . esc_attr( $value ) . '"' . $extra . ' />'
			. '</p>';
	}
}
